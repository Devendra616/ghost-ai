...
- only invite emails that belong to registered Clerk users
...
Invite requests must verify the target email belongs to an existing Clerk user before writing a collaborator record. If no registered user exists for the email, return a relevant error and do not create the collaborator.
